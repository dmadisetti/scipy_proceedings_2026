...
pyproject: |-
  ...
  dependencies = [
      ...
      "diskcache==5.6.3",
      "joblib>=1.4",
      # pymandala 0.2.0a0 ships Aleksandar Makelov's mandala lib
      # (the PyPI name `mandala` is a different project). Its metadata
      # under-declares, so the deps below are imported unconditionally
      # on `from mandala.imports import ...`.
      "pymandala>=0.2.0a0",
      "pyarrow>=15",
      "tqdm",
      "prettytable",
...
```{raw:typst}
// Hide marimo cell source blocks from the PDF render. `hide_code="true"`
// in the fence controls the live editor only — this rule strips the
// emitted code blocks at the typeset layer. Scoped to this paper via a
// raw:typst inject (kept out of the shared scipy template). Any block
// authored with a different language tag (e.g. `python-show`) falls
// through to the template's default styled code box.
...
# Notebook-only view of Figure 1. The PDF embeds the composed image
# through the myst figure directive above, which mo.md renders as raw
# text; this cell shows the same file to notebook readers. Python cell
# outputs never enter the PDF build.
